#!/usr/bin/env python3
"""Measure Vagrant's synchronous libds ownership seam from the owned PS-EXE.

The shipping override directly owns DsControlB, the blocking libds control wrapper, while retaining
its generated body for A/B use. Async DsCommand/DsPacket, callbacks, query results, reads, and XA
remain guest-owned.
...
def unique_shape(img, name, shape):
    ...
    if len(matches) != 1:
        ...
        raise Refuse(f"{name}: scanned {scanned} word-aligned candidates, matched {len(matches)} "
...
def measure(img, verify_identity=True):
    if verify_identity and img.sha1() != FIXTURE_SHA1:
        raise Refuse(f"{img.path}: sha1 {img.sha1()} != SLUS_010.40 {FIXTURE_SHA1}; "
    ...
    disk_reset, disk_scanned = unique_shape(img, "_diskReset", {
        0x0C: 0x24040009, 0x10: 0x00002821, 0x18: 0x00A03021,
        0x1C: 0x1040FFFC, 0x20: 0x24040009,
        0x54: 0x2404000E, 0x60: 0x00003021, 0x64: 0x1040FFFC,
        0x68: 0x2404000E,
    })
    ...
    if jal_target(disk_reset + 0x5C, img.r32(disk_reset + 0x5C)) != ds_control_b:
        raise Refuse("_diskReset: Pause and Setmode do not call the same blocking control wrapper")
    ...
    expected = {
        0x0C: 0x308400FF, 0x10: 0x00003021, 0x14: 0x00003821,
        0x28: 0x16000003, 0x40: 0x304200FF, 0x44: 0x1040FFFC,
        0x4C: 0x38420002, 0x50: 0x2C420001,
    }
    bad = [(off, img.r32(ds_control_b + off), want) for off, want in expected.items()
           if img.r32(ds_control_b + off) != want]
    if bad:
        off, got, want = bad[0]
        raise Refuse(f"DsControlB 0x{ds_control_b:08X}: contract mismatch at +0x{off:X}: "
    ...
    cd_command, command_scanned = unique_shape(img, "CD_cw(com,param,result,mode)", {
        0x10: 0x00A08021, 0x18: 0x00C0B021, 0x20: 0x00E09021,
        0x28: 0x00808821, 0x44: 0x322200FF, 0x48: 0x00021080,
    })
    ...
    cd_sync, sync_scanned = unique_shape(img, "CD_sync(mode,result)", {
        0x00: 0x27BDFFC0, 0x04: 0xAFB60030, 0x08: 0x0080B021,
        0x0C: 0xAFB70034, 0x10: 0x00A0B821,
    })
    sync_helper = jal_target(ds_sync + 0x1C, img.r32(ds_sync + 0x1C))
    helper_calls = [jal_target(sync_helper + off, img.r32(sync_helper + off))
                    for off in range(0, 0x30, 4) if img.r32(sync_helper + off) >> 26 == 3]
    if cd_sync not in helper_calls:
        shown = ", ".join(f"0x{x:08X}" for x in helper_calls) or "none"
        raise Refuse(f"DsSync helper 0x{sync_helper:08X}: scanned 12 instructions; calls {shown}, "
                     f"not measured CD_sync 0x{cd_sync:08X}")

    return {
        "diskReset": disk_reset, "DsControlB": ds_control_b, "DsCommand": ds_command,
        "DsSync": ds_sync, "CD_cw": cd_command, "CD_sync": cd_sync,
        "disk_scanned": disk_scanned, "command_scanned": command_scanned,
        "sync_scanned": sync_scanned,
    }


def check_config(measured, text):
    m = re.search(r"kDsControlB\s*=\s*(0x[0-9A-Fa-f]+)", text)
    if not m:
        raise Refuse(f"{OWNER}: did not find kDsControlB; cannot compare shipping owner")
    got, want = int(m.group(1), 0), measured["DsControlB"]
    ok = got == want
    print(f"  [{'ok' if ok else 'FAIL':>4}] DsControlB shipped=0x{got:08X} measured=0x{want:08X}")
    if not ok:
...
def selftest(img, measured):
    ...
    check_config(measured, open(OWNER).read())
    checks += 1
    ...
    changed = text.replace(f"kDsControlB = 0x{measured['DsControlB']:08X}",
                           f"kDsControlB = 0x{measured['DsControlB'] + 4:08X}", 1)
    if changed == text:
...
def main(argv):
    ...
    if len(args) > 1:
        print("usage: re_cd.py [--check-config] [--selftest] [SLUS_010.40]", file=sys.stderr)
        return 2
    try:
        img = Image(args[0] if args else DEFAULT_EXE)
        m = measure(img)
        print("== Vagrant synchronous libds ownership seam ==")
        print(f"  _diskReset 0x{m['diskReset']:08X} -> DsControlB 0x{m['DsControlB']:08X}")
        print(f"  DsControlB -> DsCommand 0x{m['DsCommand']:08X} + DsSync 0x{m['DsSync']:08X}")
        print(f"  measured CD_cw  0x{m['CD_cw']:08X} ({m['command_scanned']} candidates scanned)")
        print(f"  measured CD_sync 0x{m['CD_sync']:08X} ({m['sync_scanned']} candidates scanned)")
        print("  boundary: libds queue/IDs/callbacks/results remain guest-owned; reads/XA are NOT owned here")
        if do_check:
            check_config(m, open(OWNER).read())
        if do_selftest:
            selftest(img, m)
        return 0
