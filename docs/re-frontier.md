...
- gap: Resident VBlank delivery is resolved. The runtime now completes `_loadMenuSound` and the TITLE.PRG read, then stops at the separate un-emitted overlay target `0x80071334`. RE-05 has measured the later overlay frame owner statically, but no gameplay or overlay execution is claimed.
