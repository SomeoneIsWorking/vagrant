...
substrate to execute them on. A framework defect found while measuring them (issue #3: `crt0_setup`
never sets `a1`, so this game's BIOS `InitHeap` would build a zero-size heap) is outstanding upstream.
...
- notes: FRAMEWORK DEFECT found while measuring this step, recorded in docs/issues/0003: the pinned framework's crt0_setup (external/psxport/runtime/recomp/native_boot.cpp) sets c->r[4] and dispatches libcInit but never sets c->r[5]. Here libcInit is BIOS A0:0x39 InitHeap and psxport's own HLE implements it as heapInit(a0,a1) (runtime/recomp/hle.cpp:355), so the arena would be created with size 0. The guest crt0 provably passes the size in a1 (0x8001F5A0 subu $a1,$a1,$a0 -> 0x001BBE50, live into the jal at 0x8001F5CC). Fix belongs in the framework dev clone, NOT worked around game-side. SEVERITY CORRECTED 2026-08-12: it was recorded as making "every heapAlloc return 0" for THIS game, which overstated it — nothing in this image can call BIOS malloc (gap (2) above has the census), so here the wrong-size arena is inert. Fix it upstream because the contract is wrong; do not expect this port to exhibit it. ALSO CORRECTED: the 22-line disassembly block in game_config.cpp was RETYPED, not pasted — three of its raw words did not match the executable (0x8001F548 read 24427836 for a real 24423678, 0x8001F588/8C read 002420c0/002420c2 for real 000420c0/000420c2) while being presented as the audit trail CLAUDE.md rule 1 requires. It is now GENERATED by `re_crt0.py --emit-citations` and gated by `--gate-citations`; a retyped word cannot survive.
