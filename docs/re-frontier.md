...
## THE STATE OF THIS PORT, 2026-08-21: TITLE loads; overlay emission is next
...
`ce2c83ad`, completes `_initRand`. RE-04 now owns blocking `DsControlB` at `0x80025BE4`; a bounded run completes
...
callback, dispatches each queued Pause, and observes Pause completion status `0x02`. Resident `jal`
`0x80042BD8` then reaches direct TITLE target `0x80071334`. That call fails fast because no TITLE
overlay was emitted or registered. This is a real overlay-routing boundary, not evidence of a failed
field clock or CD poll, and it is not a gameplay or reached-presenter claim.
...
- gap: This is the verified resident bootstrap frontier, not a gameplay claim. Overlay modules are not in this smallest substrate. Blocking `_diskReset`, boot SPU DMA completion, resident VBlank waits, four WAVE reads, and the TITLE.PRG read now advance. The next miss, `0x80071334`, is proven overlay code and must be handled by RE-03/RE-05 overlay emission and routing, never by adding a resident seed. Future `[recomp-MISS]` failures may extend an explicit seed list only with image provenance and runtime rationale.
...
- gap: The mappings are complete. Runtime now transfers all 271 TITLE.PRG sectors and calls `0x80071334` inside its measured `0x80068800` slot. The missing piece is generated overlay emission/registration and runtime routing; adding the target to the resident image would be incorrect because those bytes belong to TITLE.PRG.
...
- gap: The presenter protocol and BATTLE buffer ownership are measured, and TITLE.PRG is now live in guest RAM, but the first direct TITLE call `0x80071334` cannot dispatch because that overlay has not been emitted or registered. TITLE's caller-provided OT allocation is not yet classified, and no game-owned runtime seam may be installed until overlay emission/routing and a reached-call gate exist.
