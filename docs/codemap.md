...
| Static recompilation | `game/recomp_seeds.json` → `generated/` | ⬜ | **Not started, and blocked on RE.** The seed file is empty. BATTLE/BATTLE.PRG's base is measured at `0x80068800` (RE-03), but the other 20 non-empty `.PRG` bases are unknown; that is not enough to emit a substrate. `emit.py` refuses a missing overlay base by design; a guessed one emits a whole module at wrong addresses. |
