...
**This repo has a verified first TITLE splash, not a rendered game.** Provisioning, the inherited title runtime,
...
disabled-producer control remains black and emits no second present. TITLE then switches to 24-bit
and stalls in its intro/MDEC/FMV path without another presentation. That next picture, later CD/XA,
the other overlays, the title menu, and gameplay remain open (RE-13).
...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (recorded pin `ad5cf802`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. **Not ours** — fix framework bugs upstream in the workspace dev clone, never in this shared checkout. |
...
| Inherited title runtime | `game/core/vagrant_runtime.{h,cpp}`, `game/core/vagrant_context.h` → `vagrant::VagrantRuntime` | ✅ | Process-lifetime derived owner of the direct-native project default, measured guest-main dispatch, CD/VBlank override composition, and the per-Core TITLE startup producer. Explicit user/harness render selections remain higher CVar layers. It derives the bounded adapter only while generic framework algorithms consume legacy facts/callbacks; its focused test proves install identity/context and that behavior no longer lives in `GameHooks`. |
...
| Guest frame/present contract | `tools/re_frame.py`, `tools/re_title_startup.py`, `game/render/title_startup.cpp`, `game/render/title_startup_recipe.cpp` | 🟡 | SHA-bound TITLE/BATTLE bytes derive later guest presenters `0x80071A68`/`0x8007629C`, resident parity/env globals, and BATTLE's dynamic heap OT/pools. RE-12 additionally derives immediate leaf `0x8006A778`, packet `0x800DED28`, splash owner `0x8006F54C`, and both calls; its retained-super semantic producer renders the first publisher splash. The later 24-bit intro presentation remains RE-13. |
| Process entry | `game/core/main.cpp` | 🟡 | Installs one process-lifetime `VagrantRuntime`, then executes through resident boot, five initial file reads, routed `vs_title_exec`, and the first native publisher/developer splash. It next enters 24-bit MDEC work without another presentation; title menu/gameplay are not observed. |
...
| Project launcher | `run.sh` → `tools/run.py`, `tools/extract_overlays.py`, `tools/ensure_recomp.py` | 🟡 | The zero-argument route provisions both executable and TITLE from the owned disc, hash-gates regeneration, builds with verified Clang, and launches headlessly. It now renders the first publisher splash; the 24-bit intro and later title sequence remain open. |
...
| Everything else about the game | — | ⬜ | The 24-bit intro/MDEC producer, title menu, later CD/XA, the other overlays, gameplay, and audio playback ownership are open. Boot through the first publisher/developer splash is the verified exception. `docs/re-frontier.md` is the ordered list. |
