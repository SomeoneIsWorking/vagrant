...
each queued Pause, and finish all five loads. Resident `jal 0x80042BD8` then calls the loaded TITLE
address `0x80071334`, but no generated overlay module owns it, so dispatch fails fast. Later CD/XA,
overlay execution, live presentation, and gameplay remain open. The TITLE/BATTLE presenter contract
is measured statically; it is not claimed reached.
...
| Guest frame/present contract | `tools/re_frame.py` → `game/core/game_config.cpp` | 🟡 | SHA-bound TITLE and BATTLE bytes uniquely derive guest presenters `0x80071A68`/`0x8007629C`, resident parity/env globals, BATTLE's OT submit owner, and its guest-heap allocations: two 0x2088 OT blocks and two 0x20000 packet pools. This proves the legacy fixed-base frame fields must remain zero; the 4/4 both-answer gate enforces it. Runtime loads TITLE.PRG but cannot yet execute its first call because the overlay module is not emitted or registered, so no presenter ownership is wired. |
| Process entry | `game/core/main.cpp` | 🟡 | Built and executed through crt0, guest main, `_initRand`, `_diskReset` Pause/Setmode, boot SPU DMA, resident VBlank, GPU-standard setup, four complete asynchronous WAVE reads, and the complete TITLE.PRG read. The next direct TITLE function fails fast because the overlay is not emitted or registered; no presenter or gameplay is observed. |
...
| Everything else about the game | — | ⬜ | Later CD/XA and streaming modes, overlay execution, live frame integration, renderer, audio playback ownership, and other native ownership: **not started.** Boot, blocking libds control, the proven async sector/data-callback and command-arbitration route, resident filesystem loads through TITLE.PRG, SPU DMA completion, resident VBlank delivery, the measured pad delivery seam, and the static guest-frame contract are the exceptions described above. `docs/re-frontier.md` is the ordered list. |
