...
**This repo has a verified TITLE-overlay bootstrap, not a rendered game.** Provisioning, the framework seam,
...
| Process entry | `game/core/main.cpp` | 🟡 | Executes through resident boot, five initial file reads, routed `vs_title_exec`, GPU DMA/title image work, and interrupt-time CD work. The old overlay miss is resolved. A non-black title frame and gameplay are not observed. |
| Build and C++ quality | `CMakeLists.txt`, `cmake/vagrant_port.cmake`, `external/psxport/tools/check_cpp_style.py` | ✅ | Clang builds `vagrant_port`; CTest covers CD classification, launcher orchestration, 4-case overlay provisioning/completeness, and the shared clang-format/clang-tidy gate. Generated/external trees are excluded. |
