...
**This repo has a verified TITLE-overlay bootstrap, not a rendered game.** Provisioning, the inherited title runtime,
...
TITLE function `0x80071334` and reaches its GPU/image and CD work. Headless runs still produce only
the initial black present before timing out inside TITLE initialization; direct native graphics,
later CD/XA, the other overlays, a non-black title frame, and gameplay remain open (issue #17/RE-12).
...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (recorded pin `7f5d3f13`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. **Not ours** — fix framework bugs upstream in the workspace dev clone, never in this shared checkout. |
...
| Inherited title runtime | `game/core/vagrant_runtime.{h,cpp}` → `vagrant::VagrantRuntime` | ✅ | Process-lifetime derived owner of the direct-native project default, measured guest-main dispatch, and CD/VBlank override composition. Explicit user/harness render selections remain higher CVar layers. It derives the bounded adapter only while generic framework algorithms consume legacy facts/callbacks; its focused test proves install identity and that behavior no longer lives in `GameHooks`. |
...
| Guest frame/present contract | `tools/re_frame.py` → `game/core/game_config.cpp` | 🟡 | SHA-bound TITLE and BATTLE bytes uniquely derive guest presenters `0x80071A68`/`0x8007629C`, resident parity/env globals, and BATTLE's dynamic heap OT/pools. TITLE now executes but 15/30-second headless runs produce no non-black game frame. RE-12 must classify initialization to first present and introduce direct native production. |
| Process entry | `game/core/main.cpp` | 🟡 | Installs one process-lifetime `VagrantRuntime`, then executes through resident boot, five initial file reads, routed `vs_title_exec`, GPU DMA/title image work, and interrupt-time CD work. The old overlay miss is resolved. A non-black title frame and gameplay are not observed. |
| Build and C++ quality | `CMakeLists.txt`, `cmake/vagrant_port.cmake`, `external/psxport/tools/check_cpp_style.py` | ✅ | Clang builds `vagrant_port`; CTest covers runtime inheritance/ownership, CD classification, launcher orchestration, 4-case overlay provisioning/completeness, and the shared clang-format/clang-tidy gate. Generated/external trees are excluded. |
| Project launcher | `run.sh` → `tools/run.py`, `tools/extract_overlays.py`, `tools/ensure_recomp.py` | 🟡 | The zero-argument route provisions both executable and TITLE from the owned disc, hash-gates regeneration, builds with verified Clang, and launches headlessly. Its current target executes TITLE but does not yet provide a verified non-black native picture. |
...
| Everything else about the game | — | ⬜ | Direct native graphics producers, a non-black title frame, later CD/XA, the other overlays, gameplay, and audio playback ownership are open. Boot through early TITLE initialization is the verified exception. `docs/re-frontier.md` is the ordered list. |
