...
## Read this before anything else: the honest state, 2026-08-20

**This repo has a verified resident bootstrap, not gameplay.** Provisioning, the framework seam,
registries, and the matching CC0 reference are joined by four measured groups: crt0/boot (RE-01), a
...
(RE-06). Against pinned psxport `be381503`, `vagrant_port` executes crt0 and guest main, completes
`_initRand`, and completes `_diskReset` through measured `DsControlB` ownership. The next watchdog is
sampled later under generated `0x8001355C`. Async CD,
...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (recorded pin `be381503`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. **Not ours** — fix framework bugs upstream in the workspace dev clone, never in this shared checkout. |
...
| Everything else about the game | — | ⬜ | Async CD/read/XA, frame loop, renderer, audio, and other native ownership: **not started.** Boot, blocking libds control, and the measured pad delivery seam are the exceptions described above. `docs/re-frontier.md` is the ordered list. |
