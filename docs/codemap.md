...
next reached boundary is missing TITLE function `0x800798A4`; normal movie completion,
...
| Process entry | `game/core/main.cpp` | 🟡 | Installs one process-lifetime `VagrantRuntime`, then executes through resident boot, five initial file reads, routed `vs_title_exec`, publisher/developer splashes, live 24-bit intro frames, Start skip, and the first 15-bit title-menu picture. The next fail-fast is missing TITLE function `0x800798A4`; normal movie completion, teardown, later interaction, and gameplay are not observed. |
...
| Everything else about the game | — | ⬜ | Normal intro completion, XA/STR teardown, later title-menu interaction, later CD/XA, the other overlays, gameplay, and audio playback ownership are open. Boot through a forced Start skip to the first readable title menu is the verified exception. `docs/re-frontier.md` is the ordered list. |
