...
replay run enters generated BATTLE `0x800798A4` and INITBTL `0x800FA35C`, then exposes the next
cross-overlay callable-entry defect at BATTLE `0x800E6EAC`; normal movie completion, XA/STR teardown,
later menu interaction, other overlays, and gameplay remain open.
...
| Process entry | `game/core/main.cpp` | 🟡 | Installs one process-lifetime `VagrantRuntime`, then executes through resident boot, five initial file reads, routed `vs_title_exec`, publisher/developer splashes, live 24-bit intro frames, Start skip, the first 15-bit title-menu picture, and BATTLE/INITBTL entry. RE-15 proves generated BATTLE `0x800798A4` and INITBTL `0x800FA35C` execute. The next boundary is the missing separate BATTLE callable entry at `0x800E6EAC` (issue #22). |
