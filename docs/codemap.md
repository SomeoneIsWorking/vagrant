...
registries, and the matching CC0 reference are joined by five measured groups: crt0/boot (RE-01), a
...
(RE-06) and boot SPU DMA completion route (RE-09). Against pinned psxport `2b5ef7b5`, `vagrant_port`
executes crt0 and guest main, completes `_initRand` and `_diskReset`, then dispatches the measured
DMA4 callback. The old `0x8001355C` wait clears; the next watchdog is Sony `VSync`
(`0x8001F6C4` → `0x8001F83C`). Async CD,
...
| Process entry | `game/core/main.cpp` | 🟡 | Built and executed through crt0, guest main, `_initRand`, `_diskReset` Pause/Setmode, and boot SPU DMA. The current honest boundary is a later watchdog in Sony `VSync` (`0x8001F6C4` → `0x8001F83C`); no gameplay/overlay loading is observed. |
...
| Everything else about the game | — | ⬜ | Async CD/read/XA, VBlank/frame loop, renderer, audio playback ownership, and other native ownership: **not started.** Boot, blocking libds control, SPU DMA completion, and the measured pad delivery seam are the exceptions described above. `docs/re-frontier.md` is the ordered list. |
